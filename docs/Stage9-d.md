...
**Resolution:** использовать текущий код как source of truth и ввести explicit constants/helpers там, где metas критичны. `ItemResource`, `ItemNugget`, `ItemShard`, `blockMetalDevice`, `blockCosmeticSolid`, `blockCustomPlant`, `blockTaint` — это как раз те места, где "магические числа" из reference могут убить порт.

Формулировка: "Crucible recipe metadata must be resolved through current-port constants/helpers or verified subtype mappings. Reference metadata should not be pasted directly unless it matches the current 1.12.2 item/block definitions."

### Q2: Где живёт equivalent `ConfigResearch.recipes`?

**Resolution:** этот вопрос уже устарел: он живёт в `ConfigResearch`. В текущей ветке `ConfigResearch` уже содержит `public static final Map<String, Object> recipes`, а `ConfigRecipes.init()` уже очищает и наполняет эту map. Правильное решение — не создавать вторую map, а укрепить существующую: typed helper methods, validators, единый lifecycle.

Формулировка: "The current port already has `ConfigResearch.recipes`; Stage 9-d should use it as the compatibility bridge for research page recipe references. Do not create a second recipe map. Add typed helpers or validators if needed to avoid unsafe raw `Object` usage."

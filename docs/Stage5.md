...
Текущий wand хранит vis как `vis_` + aspect tag: `src/main/java/thaumcraft/common/items/wands/ItemWandCasting.java:39-43`, `src/main/java/thaumcraft/common/items/wands/ItemWandCasting.java:85-94`. Reference хранит primal vis прямо по aspect tag (`aer`, `terra`, и т.п.) в NBT. Это несовместимо с оригинальными saved stacks и с любым кодом, который ожидает старые ключи.
...
Восстановить wand core как поведенческий центр Stage 5: NBT compatibility, focus cooldown, focus potency/treasure/enlarge/extend helpers, staff/sceptre/rune behavior, block/tile trigger routing and server/client side boundaries.

**Как доделать:**
- `src/main/java/thaumcraft/common/items/wands/ItemWandCasting.java`: привести vis NBT к reference semantics или добавить явную миграцию с `vis_` на original keys без потери текущих stacks; восстановить `getAllVis`, `getAspectsWithRoom`, `storeAllVis`, `storeVis`, `consumeVis`, focus helper methods, staff/sceptre/rune methods, creative presets.
...
**Критерии приемки:**
- [ ] Original wand NBT keys for rod/cap/focus/upgrades/vis are readable and newly written stacks remain compatible or have documented migration.
...
- Нужно решить NBT strategy для wand vis keys: писать строго reference keys или поддержать одноразовую миграцию текущих `vis_` stacks. Это не заменяет GAP-1; без решения wand parity blocked.
- Нужно подтвердить, какие item GUI ids уже заняты в текущем порте для Focus Pouch, Hand Mirror and Thaumonomicon before implementation.
- Нужно подтвердить наличие или отсутствие 1.12 `TileVisRelay` equivalent before closing Vis Amulet relay behavior.
- Нужно определить минимальный Phase 8 boundary для item models/lang versus dynamic focus/Thaumometer FX; static item resources should not be deferred if Stage 5 closure is claimed.

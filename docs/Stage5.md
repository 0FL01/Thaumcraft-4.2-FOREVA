...
- чтение должно быть совместимым: сначала читать reference key, затем legacy key `vis_` + aspect tag для stacks, уже созданных текущим портом;
- при любом изменении vis нужно мигрировать значение в reference key и удалить либо игнорировать legacy key для этого aspect;
- не писать оба формата постоянно, чтобы не оставить постоянный риск рассинхрона.
...
2. `port: restore wand and pouch NBT compatibility` — wand vis migration, 18-slot Focus Pouch `Inventory`/`Slot` NBT, Baubles focus pouch shared inventory.

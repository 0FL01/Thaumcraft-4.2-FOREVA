...
- [ ] Port or explicitly defer all missing Stage 4 block/tile pairs from reference `ConfigBlocks`, prioritizing tubes, mirror, reservoir block and arcane furnace.
- [ ] Port tube block/tile family or document exact Stage 4 blocker if postponed.
- [ ] Implement `TileMirrorEssentia` and mirror block if essentia mirror remains in Stage 4 scope.
- [ ] Implement or explicitly defer `BlockArcaneFurnace`, `TileArcaneFurnace`, and `TileArcaneFurnaceNozzle`.
...
- Нужно ли включать в Stage 4 closure полный набор reference-блоков из `thaumcraft_src/thaumcraft/common/config/ConfigBlocks.class` (`blockChestHungry`, `blockMagicBox`, `blockLifter`, `blockMirror`, `blockTube`, `blockEssentiaReservoir`, `blockAlchemyFurnace`) или часть должна быть явно перенесена в другой checkpoint как dependency? Пока это не решено, Stage 4 нельзя закрывать.
- Какая текущая 1.12 API-точка должна заменить reference `VisNetHandler.drainVis(... Aspect.ENTROPY ...)` для Arcane Bore: существующая Phase 3 aura/vis реализация или прямой essentia-only режим через Bore Base? Без этого нельзя завершить Bore parity.
- Какие минимальные recipe/research fixtures использовать для Stage 4 validation, чтобы не ждать полного Phase 9 content parity, но проверить Thaumatorium, Crucible and Infusion server flow?

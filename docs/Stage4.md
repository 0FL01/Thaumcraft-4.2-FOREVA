...
Исторический список ниже оставлен для traceability. Активный closure status после checkpoint см. в разделах 6 и 9; unchecked acceptance boxes внутри отдельных GAP-секций не являются текущим task list после fresh-world closure.
...
- [x] Register metadata-preserving ItemBlocks for `stone_device`, `wooden_device`, `metal_device` and other required Stage 4 metadata blocks.
- [x] Make Arcane Bore server container functional: restricted focus/pickaxe slots, player inventory, shift-click.
- [x] Restore Arcane Bore base ENTROPY essentia transport and Bore vis/essentia consumption parity or documented compatible equivalent.
- [x] Make Thaumatorium server container functional: input slot, recipe candidate list, research-aware recipe toggling, shift-click.
- [x] Implement `TileThaumatoriumTop` delegation for inventory, aspects and essentia transport.
- [x] Make Focal Manipulator server container functional: focus slot, button handling, shift-click and failure feedback.
- [x] Audit NBT and update sync for touched core tiles: Bore/Base, Thaumatorium/Top, Focal Manipulator and metadata block placement paths.

### 6.2 Transport closure — minimal server essentia network

- [x] Port or explicitly defer all missing Stage 4 block/tile pairs from reference `ConfigBlocks`, prioritizing tubes, reservoir block and alembic/server transport.
- [x] Port tube block/tile family or document exact Stage 4 blocker if postponed.
- [x] Register and validate `BlockEssentiaReservoir` or document an exact deferral; fix `TileEssentiaReservoir` one-face side semantics and active fill behavior if retained in Stage 4.
- [x] Audit and validate alchemy furnace + alembic + centrifuge + reservoir transport behavior.

### 6.3 Explicit deferrals allowed for pre-Phase8 transition

- [x] Implement or explicitly defer `TileMirrorEssentia` and mirror block; mirror gameplay may move to a later Stage 4 transport/content checkpoint if linking item flow is outside current scope.
- [x] Implement or explicitly defer `BlockArcaneFurnace`, `TileArcaneFurnace`, and `TileArcaneFurnaceNozzle`; do not count registered stub tiles as parity.
- [x] Explicitly classify `blockChestHungry`, `blockMagicBox`, `blockLifter`, `TileMagicBox`, `TileChestHungry`, `TileLifter`, `TileBrainbox`, `TileMemory`, and `TileEssentiaCrystalizer` as required, dependency-gated, or deferred before claiming full Stage 4 parity.

### 6.4 Validation closure

- [x] Audit `TileInfusionMatrix` against reference lifecycle, NBT, instability and reload behavior; runtime recipe coverage remains Phase 9/content-dependent.
- [x] Audit `TileCrucible` against reference heating, fluid, smelting, spill and entity collision behavior; broad recipe coverage remains Phase 9/content-dependent.
- [x] Audit Portable Hole wrapper restore behavior with normal blocks and tile entity restore code paths; interactive focus scenario is Phase 5/client-manual dependent.
- [x] Audit Warding wrapper owner/security/restoration and stored block delegation; interactive focus scenario is Phase 5/client-manual dependent.
- [x] Audit NBT and update sync for all Stage 4 tiles.
- [x] Run `./scripts/dev.sh compileJava` after implementation changes.
- [x] Run `./scripts/dev.sh smoke-server` after registry/tile/block changes.
- [x] Record manual/server Stage 4 scenario notes for: place all required blocks, open server containers, save/reload, break/recover inventories, operate Crucible, Infusion Matrix, Arcane Bore, Thaumatorium, Focal Manipulator, essentia network, Portable Hole, Warding.
- [x] Update `docs/Stage4.md` after implementation/validation and remove resolved blocker/high gaps.

## 7. Definition of Done

Stage 4 fresh-world server/common closure считается завершенной для перехода к Phase 8 только если:
...
Открытые вопросы после Stage 4 closure:

- Dev/manual validation fixtures остаются Phase 9/test-harness вопросом: они не должны попадать в normal content registration и маскировать Phase 9 gaps.
- `TileEssentiaCrystalizer` не вошел в этот transport closure и классифицирован как deferred reference piece в разделе 9.4.

## 9. Closure notes — 2026-05-14 fresh-world server/common

### 9.1 Implemented closure scope

- Metadata-preserving ItemBlocks are registered for `blockTable`, `blockStoneDevice`, `blockWoodenDevice`, `blockMetalDevice`, `blockMagicalLog`, `blockTube`, `blockMirror`, and `blockEssentiaReservoir`. Existing 1.12 registry names remain legacy-lowercase resource paths such as `thaumcraft:blockstonedevice`; no snake_case rename was introduced.
- Stage 4 device placement reachability is restored for core metadata variants: alchemy furnace, pedestal, infusion matrix/pillar, wand pedestal, node stabilizer/converter, spa, focal manipulator, flux scrubber, bellows, bore base, bore, arcane pressure/sensor/banner variants, crucible, alembic, grates, lamps, thaumatorium bottom/top, vis relay, tubes, reservoir, and mirrors.
- Server containers now have slots and shift-click routing for Arcane Bore, Thaumatorium, Focal Manipulator, and Alchemy Furnace. Thaumatorium button handling rebuilds candidate `CrucibleRecipe` entries from current API recipes and respects research keys when present; blank-key/dev recipes remain usable for manual fixtures.
- `TileThaumatoriumTop` delegates inventory, aspect container, and essentia transport calls to the bottom `TileThaumatorium` at `pos.down()`.
- Arcane Bore base now implements `IEssentiaTransport`, suctions `ENTROPY`, and exposes `drawEssentia()`. Arcane Bore persists `SpeedyTime`, drains `ENTROPY` vis through `VisNetHandler`, falls back to base-drawn essentia, and slows mining when the speed budget is empty.
- Server essentia transport baseline now includes `BlockTube`, `TileTube`, `TileTubeValve`, `TileTubeFilter`, `TileTubeBuffer`, `TileTubeRestrict`, and `TileTubeOneway`, plus reservoir one-face suction/fill behavior, alembic side restrictions/filter NBT, thaumatorium top transport, and essentia mirror link/drain behavior.
- Fresh-world NBT writes for Portable Hole and Warding wrappers use registry/resource names (`oldblockName`, `blockName`) instead of writing old numeric block ids. Existing numeric read fallbacks were removed from the touched wrapper paths.
- Common tile initial sync now uses `TileThaumcraft.getUpdateTag/handleUpdateTag`; Pedestal writes the reference `Items` key; Warded blocks deny normal harvest and retain no-drop/explosion protection.

### 9.2 Validation run

- `./scripts/dev.sh compileJava` — passed after implementation changes.
- `./scripts/dev.sh smoke-server` — passed; Forge reached `Done (` with no crash markers.
- `./scripts/dev.sh build` and `./scripts/dev.sh check-jar` — not run because this checkpoint did not produce a release/runtime jar for Prism/normal Forge distribution.

### 9.3 Manual/server scenario notes

- Placement: server registry and metadata ItemBlock paths are present for required Stage 4 block variants. Full interactive placement of every variant was not run in this headless session; fresh-world server smoke verified registration/load reachability.
- Activation/containers: server GUI handlers route to functional containers for Arcane Bore, Thaumatorium, Focal Manipulator, and Alchemy Furnace. Phase 8 client GUI screens remain deferred, so interactive GUI opening was not client-smoked here.
- Tile NBT reload: touched tiles have save keys for inventories, facing/orientation, recipe hashes/players, `SpeedyTime`, reservoir aspects/facing, tube open/choke state, mirror links, and wrapper registry names. Dedicated save/reload manual scenarios still need interactive harness coverage before final parity polish.
- Essentia transfer: server code supports alembic/tube/reservoir/thaumatorium/bore-base/mirror movement and suction contracts. Broad throughput/edge-case validation remains dependent on Phase 9 content fixtures or a dedicated test harness.
- Portable Hole restore: wrapper countdown/plane/restore code was audited against reference and now writes registry-name block identity. Focus-triggered live restore through player use remains a Phase 5/manual scenario.
- Warding owner/security: wrapper NBT, owner hash, no-drop, explosion protection, harvest denial, and stored-state delegation were audited. Owner/non-owner ward/unward runtime still depends on Focus Warding player interaction and remains a manual smoke item.
- Crucible/Infusion: server baselines remain in scope and were audited for NBT/sync risk. Full recipe completion/instability validation is content-dependent and stays Phase 9/manual validation, not Phase 8 client polish.

### 9.4 Explicit remaining dependencies and deferrals

- Phase 8: client GUI screens, TESR/model/FX/shader polish, Portable Hole visuals, Warding visuals, tube rendering/highlight detail, and machine GUI presentation.
- Phase 9: complete recipe/research/content registration and broad Crucible/Infusion/Thaumatorium recipe progression validation.
- Deferred Stage 4 reference pieces not required for this fresh-world pre-Phase8 server/common closure: `BlockArcaneFurnace`, `TileArcaneFurnace`, `TileArcaneFurnaceNozzle`, `blockChestHungry`, `blockMagicBox`, `blockLifter`, `TileMagicBox`, `TileChestHungry`, `TileLifter`, `TileBrainbox`, `TileMemory`, `TileEssentiaCrystalizer`, and normal item mirror entity-teleport gameplay. These must not be counted as parity until explicitly ported in a later scoped checkpoint.

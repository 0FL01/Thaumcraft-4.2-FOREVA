...
Reference behavior restored on Forge 1.12.2 terms without reintroducing numeric potion IDs.
...
- Added deprecated overload `(int ignoredId, boolean isBadEffect, int color)` in both API potion classes; the numeric id is ignored and does not affect Forge registry identity.
- Restored `static init()` as idempotent name/icon/effectiveness setup.
- `Config.initPotions()` assigns `PotionFluxTaint.instance` / `PotionVisExhaust.instance` to the registered 1.12 instances before registry events.
- Restored `isInstant()`, icon indexes `(3, 1)` and `(5, 1)`, and effectiveness `0.25D`.
- Copied original `textures/misc/potions.png` from `thaumcraft_src/assets/`.
...
**Критерии приемки:**
- [x] Raytrace result `hitVec` remains the precise intersection point for cuboids.
- [x] Raytrace result block position equals the target `BlockCoord`.
- [x] `ExtendedMOP` still preserves `data` and distance ordering.
- [ ] Runtime/manual check validates selection/collision behavior on a CCL cuboid model; deferred to `GAP-6` client/manual scene.
...
**Критерии приемки:**
- [x] `javap -public thaumcraft.codechicken.lib.math.MathHelper` shows `public static int compare(double, double)`.
- [x] All current call sites compile without casts or behavior changes.
- [x] `compileJava`, `apiJar`, and `devJar` pass.
...
**Статус:** реализовано для Stage 1 minimum backend
**Критичность:** high
...
The original delegation shape is restored behind the internal method handler, with a Stage 1 minimum backend in `ThaumcraftCraftingManager`.
...
- `InternalMethodHandler.getStackInRowAndColumn()` now delegates to `TileMagicWorkbench.getStackInRowAndColumn()` when given the 1.12 tile instance.
- `InternalMethodHandler.getObjectAspects()`, `getBonusObjectTags()`, and `generateTags()` now delegate to `ThaumcraftCraftingManager` instead of returning placeholders.
- `ThaumcraftCraftingManager.getObjectTags()` now handles exact meta, grouped meta, wildcard meta, safe generated fallback, wand tags, and potion-derived tags.
- `ThaumcraftCraftingManager.getBonusTags()` now handles `IEssentiaContainerItem`, base object tags, armor/tool/weapon/bow/harvest class bonuses, and enchantment-derived tags.
- `ThaumcraftCraftingManager.generateTags()` now derives bounded tags from implemented crucible, arcane, infusion, and Forge/vanilla recipe data with recursion protection and tag caps.
- `ThaumcraftApi.exists()` and smelting bonus wildcard lookups now use integer `Short.MAX_VALUE` keys compatible with `ItemStack#getMetadata()` registrations.

**Критерии приемки:**
- [x] No `Phase 3` / `Phase 4` placeholder comments or empty fallback behavior remain in active Stage 1 API-facing handler paths.
- [x] Object tag lookup covers exact, grouped, wildcard, and generated fallback paths.
- [x] Crafting/grid lookup delegates to the reference-equivalent 1.12 `TileMagicWorkbench` method.
- [x] `compileJava`, `apiJar`, `devJar`, and server smoke pass.
- [ ] Representative in-game addon/API scenarios remain manual validation work beyond this implementation checkpoint.
...
**Критерии приемки:**
- [x] No current code path references `thaumcraft.codechicken.core.launch.DepLoader`; search results are documentation-only.
- [x] Stage 1 closure notes explicitly mark legacy `DepLoader` as excluded.
- [x] Runtime server smoke does not attempt legacy dependency downloads or launch injection.
...
**Статус:** accepted validation deferral
**Критичность:** medium
...
**Критерии приемки:**
- [ ] Client smoke or manual runtime reaches a scene using CCL render helpers without crash; current client smoke fails before mod init due LWJGL display enumeration in the environment.
- [ ] Visual output for a representative helper-rendered model matches reference intent closely enough for the 1.12 renderer; deferred to Phase 8/client scene.
- [x] Raytracing helper returns separate precise hit vector and block position, with ordered `ExtendedMOP` data preserved by implementation and signature/search checks.
- [ ] TrueType text renders at expected position/scale/color on at least two GUI scale settings; deferred to Phase 8/client scene.

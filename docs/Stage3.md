...
- Existing 1.7.10 `.thaum`/`.thaumbak` research files migrate or are explicitly deferred with compatible loader semantics.
...
### GAP-1: Нет совместимого `.thaum` / `.thaumbak` persistence и migration
...
Current state persists only Forge capability NBT with keys `warpPerm`, `warpSticky`, `warpTemp`, `warpCounter`, `discoveredAspects`, `scannedEntities`, `scannedItems`, `scannedPhenomena`, `researchComplete` (`src/main/java/thaumcraft/common/lib/capabilities/PlayerKnowledgeCapability.java:230-238`). Reference stores separate compressed player files `id.thaum` and backup `id.thaumbak`, writes the original `THAUMCRAFT.*` tags, backs up old files, loads backup on failure, and migrates legacy warp keys. This directly hits the PRD known risk `Offline .thaum and .thaumbak compatibility, currently deferred` (`docs/PRD.md:249-255`).

**Что нужно доделать:**

Implement a 1.12.2-compatible loader/saver or a documented migration layer that reads original `.thaum`/`.thaumbak` data into the capability and writes equivalent data without losing original semantics.

**Как доделать:**
- Add Stage 3-compatible methods to `ResearchManager`: `loadPlayerData`, `savePlayerData`, `loadResearchNBT`, `saveResearchNBT`, `loadAspectNBT`, `saveAspectNBT`, `loadScannedNBT`, `saveScannedNBT`, `scheduleSave`.
- Map reference NBT keys into `IPlayerKnowledge` fields and add missing aspect pool storage from GAP-2.
- Hook load on player login/server player data availability and save on research/aspect/scan/warp mutation.
- Preserve backup behavior: read `.thaum`, fallback `.thaumbak`, write temp/backup safely.
...
- Scenarios: first login with old `.thaum`, failed `.thaum` with valid `.thaumbak`, server restart, player death clone, offline username lookup.

**Критерии приемки:**
- [ ] Existing 1.7.10 `.thaum` research/aspect/scan/warp data loads into the 1.12.2 capability.
- [ ] `.thaumbak` fallback works when primary `.thaum` is missing or unreadable.
- [ ] Saving preserves research, aspects, aspect pools, scans, warp and runic shielding using documented compatible keys.
- [ ] Server restart does not lose Stage 3 player knowledge.

**Риски / зависимости:**

High save-data risk. Requires careful mapping between old username-based reference data and Forge 1.12.2 player UUID/profile storage. This is a Stage 3 blocker; it should not be hidden behind a later phase.
...
- Serialize aspect pool using reference `THAUMCRAFT.ASPECTS` list entries with `key` and `amount`, or document a compatible capability key plus migration from reference.
...
Current `getResearchData(String username)` finds an online player, updates `playerDataCache`, then returns cache only (`src/main/java/thaumcraft/common/lib/research/ResearchManager.java:95-109`). It does not load offline `.thaum` data from disk. Reference maps knowledge by username and loads player data from `playersDirectory` `.thaum`/`.thaumbak` when needed.
...
**Как доделать:**
- Integrate offline load with GAP-1.
- Cache loaded offline data using normalized username and invalidate/update on player login/logout/save.
...
- Files/classes: `ResearchManager`, `PlayerKnowledge`, `InternalMethodHandler`, capability storage/migration layer.

**Критерии приемки:**
- [ ] Username research checks work for online players.
- [ ] Username research checks work for offline players with existing `.thaum` data.
...
Для закрытия Stage 3 принят вариант **Capability-primary + compatibility bridge**:

- Forge capability остается основным runtime/backend-хранилищем player knowledge.
- Legacy `.thaum` / `.thaumbak` должны читаться как migration/import source и offline fallback для username-based API.
- Save path должен писать capability state и backup-safe compatibility mirror с reference-compatible `THAUMCRAFT.*` NBT keys там, где данные однозначно мапятся.
- Полная 1.7.10 round-trip совместимость `.thaum` файлов не является Stage 3 целью; любые несовместимые или неподдержанные поля должны быть явно задокументированы без data-loss path.
...
1. `knowledge+persistence`: aspect pools, `.thaum/.thaumbak` bridge, offline username lookup, capability clone hardening.
...
- [ ] GAP-1 closed: `.thaum`/`.thaumbak` load/save/migration works or has an approved explicit deferral with no data-loss path.
...
- Решено: Stage 3 использует capability-primary persistence с `.thaum/.thaumbak` compatibility bridge. Требуется backup-safe read/import/fallback и compatible mirror save там, где данные однозначно мапятся; strict 1.7.10 round-trip write не требуется.

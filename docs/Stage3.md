...
- Нужно ли Stage 3 закрывать `.thaum`/`.thaumbak` запись в оригинальные файлы или достаточно one-way migration в Forge capability с backup-safe чтением? До решения GAP-1 остается blocker.
- Какой минимальный набор research entries допустим для Stage 3 validation без переноса полного Phase 9 content registration? До решения GAP-16 full research gameplay validation заблокирована dependency.
- Должен ли `warpCounter` синхронизироваться клиенту или оставаться server-only? Это нужно решить при закрытии GAP-6.
- Champion mob runic shielding зависит от отсутствующего/измененного champion attribute. Для Stage 3 нужно решить, закрывается ли только player runic shielding, а champion behavior остается dependency на entity systems.

...
- Latest Stage 8-e projectile follow-up restored throwable FX parity for `EntityAlumentum`, `EntityBottleTaint`, and `EntityEldritchOrb` via dedicated proxy paths (`wispFX2`, compact sparkle, taintsplosion/bottle-break, status-16 wisp burst); `validate` and `validate --smoke` passed afterward.

...
- Latest common/client follow-up restored `TileJarBrain` client-side rotation/sigh behavior used by `TileJarRenderer`, brought back `EntityPech.handleStatusUpdate(...)` tame/anger feedback via the generic particle path, and recovered item-grate open/closed parity in `BlockMetalDevice` with hand/redstone toggles, thin collision, open-item drops, and shaped `blockmetaldevice_5/_6` models; `validate` and `validate --smoke` passed afterward.

...
- Latest Stage 8-e follow-up restored `TileJarBrain` client-side rotation/sigh behavior used by `TileJarRenderer` and brought back `EntityPech.handleStatusUpdate(...)` tame/anger feedback via the generic particle path; `validate` and `validate --smoke` passed afterward.

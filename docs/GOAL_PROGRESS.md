...
- Stage 9-e burst hardened `PacketPlayerCompleteToServer` around a server-validated `processRequest(...)` path: the server now rejects wrong dimension/username payloads, rechecks requisites against the live player capability, preserves the primary-note vs secondary-completion split, and keeps sibling grants behind valid completion.
- Added `PacketPlayerCompleteRuntimeTest` for primary note creation, prerequisite rejection without input loss, secondary aspect-cost completion with sibling unlock, and wrong-type/insufficient-cost rejection; aligned the static guards with the new live-player authority contract.
- Validated this burst with `./scripts/dev.sh gradle test --rerun-tasks --tests thaumcraft.common.lib.network.playerdata.PacketPlayerCompleteRuntimeTest --tests thaumcraft.common.lib.network.playerdata.PacketPlayerCompleteToServerStaticGuardTest --tests thaumcraft.common.lib.research.ResearchClueProgressionStaticGuardTest` and `./scripts/dev.sh validate --smoke`.
- Still unverified and still forbidden to claim: normal Thaumonomicon/research-browser packet dispatch, full note solve/completion e2e, hidden/lost clue runtime closure, scan/research-table live-route parity, and `bonusAspects` save/load parity. Do not call Stage 9-e complete or server-authoritative progression complete from this checkpoint.

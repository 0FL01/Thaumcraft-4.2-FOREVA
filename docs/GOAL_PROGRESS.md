...
- Latest follow-ups moved the static advanced alchemy-furnace shell into `blockstonedevice_0` and converted golem fishing `WATER_SPLASH` send-sites from server-side vanilla particles to entity-status-driven `Thaumcraft.proxy.golemFishingSplashFX(...)` on dedicated `FXBubble`; focused validation is green and full `validate` / `validate --smoke` is the next gate.

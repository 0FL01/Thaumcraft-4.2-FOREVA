...
- Latest Stage 8 device/entity follow-ups restored the missing `LampFertility` bundle end-to-end, moved the crystal family back onto TESR/TEISR-first routing with a grouped `vcrystal.obj` path, and now swapped `RenderPech` / `RenderFireBat` off vanilla `ModelBiped` / `ModelBat` fallbacks onto dedicated `ModelPech` / `ModelFireBat` paths plus a custom Pech held-item layer; `validate` passed throughout, and the runtime-risk render bundles also passed `validate --smoke`.

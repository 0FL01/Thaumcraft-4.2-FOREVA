...
- Thaumometer pool/discovery notifications now carry the aspect description inline as well: `Aspect.getLocalizedDescription()` falls back to the legacy `tc.aspect.help.*` corpus when `tc.aspect.<tag>` is absent, and `ClientProxy` formats `%n` as `AspectName (description)` so the top-right notification never falls back to a raw `tc.addaspectpool` key again in the current en_us asset corpus.

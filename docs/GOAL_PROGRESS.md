...
- The held thaumometer renderer also returns to a dedicated first-person route. `ThaumometerPerspectiveModel` now bypasses donor first-person display matrices, and `ItemThaumometerRenderer` again applies a TC4-style equipped-progress/arm/HUD transform chain before the TC4→TC6 scanner basis adapter. Non-first-person contexts still keep the donor display transforms already validated earlier.

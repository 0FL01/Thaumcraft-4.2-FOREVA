...
- Live checkpoint prose moved into `docs/GOAL_PROGRESS-archive/` batch files; the latest validated Stage 8-c eldritch follow-ups first returned the tile-backed `BlockEldritch` family (`0/1/3/8/9`) to TESR/TEISR-first routing, then restored the original inset static shells for `meta 4/5/6` and the creative exposure contract (`meta 4` only).

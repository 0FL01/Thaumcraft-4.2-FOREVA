...
- Stage 9-e burst lifted scan/table packet coverage from helper-only tests to packet field-routing surfaces: `PacketScannedToServer`, `PacketAspectPlaceToServer`, and `PacketAspectCombinationToServer` now expose package-private dispatch helpers that preserve the real `playerid`/`dim`/`coords` gating before server-side mutation.
- Extended `PacketScannedAuthorityRuntimeTest` and `ResearchTableAuthorityRuntimeTest` to prove valid dispatch mutation plus wrong-player/wrong-dimension/wrong-table rejection on those packet routing paths; aligned the aggregate clue/scan static guard with the new dispatch helper surface.
- Validated this burst with `./scripts/dev.sh gradle test --rerun-tasks --tests thaumcraft.common.lib.network.playerdata.PacketScannedAuthorityRuntimeTest --tests thaumcraft.common.lib.network.playerdata.ResearchTableAuthorityRuntimeTest --tests thaumcraft.common.lib.research.ResearchClueProgressionStaticGuardTest` and `./scripts/dev.sh validate --smoke`.
- Still unverified and still forbidden to claim: normal Thaumonomicon/research-browser packet origin, full note solve/research-table completion e2e, broader real-content scan trigger matrix, and scan/research-table live-route parity beyond current packet field-routing coverage. Do not call Stage 9-e complete or server-authoritative progression complete from this checkpoint.

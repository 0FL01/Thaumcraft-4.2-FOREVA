...
- Stage 9-e burst moved hidden/discovery/note progression onto live-player capability checks where a real `EntityPlayer` is already present: `findHiddenResearch(...)`, `findMatchingResearch(...)`, `createClue(...)`, and `ItemResearchNotes` completion/sibling gating no longer rely on username/cache state in those live paths.
- Added `ResearchClueAndNotesRuntimeTest` to exercise hidden research selection, discovery note reveal, completed note unlock/sibling behavior, `findMatchingResearch(...)`, and `@KEY` clue-only grants; aligned the clue/matching static guards with the new live-player authority contract.
- Validated this burst with `./scripts/dev.sh gradle test --rerun-tasks --tests thaumcraft.common.lib.research.ResearchClueAndNotesRuntimeTest --tests thaumcraft.common.lib.research.ResearchClueProgressionStaticGuardTest --tests thaumcraft.common.lib.research.ResearchManagerFindMatchingResearchStaticGuardTest` and `./scripts/dev.sh validate --smoke`.
- Still unverified and still forbidden to claim: normal Thaumonomicon/research-browser packet dispatch, full note solve/research-table completion e2e, representative scan-trigger runtime coverage across real content, scan/research-table live-route parity, and `bonusAspects` save/load parity. Do not call Stage 9-e complete or server-authoritative progression complete from this checkpoint.

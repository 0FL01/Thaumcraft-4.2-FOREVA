...
- Latest node-family follow-up restored `BlockAiry` TESR/TEISR routing for metas `0/5`, recovered the airy bounds/collision/drop contract, and brought `TileNode` back onto the original lock/wait/discharge path with server-driven `PacketFXBlockZap` emission toward nearby airy nodes; `compileJava`, focused node tests, `validate`, and `validate --smoke` all passed.

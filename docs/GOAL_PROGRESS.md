...
- Latest Stage 8/9 device-content follow-up restored the missing `LampFertility` bundle end-to-end: `blockMetalDevice` meta `13` now creates/registers `TileArcaneLampFertility`, uses lamp-sized bounds plus dynamic light, routes through the shared arcane-lamp TESR, and no longer falls back to the pedestal placeholder model; focused compile/tests passed and full validation is pending for this burst.

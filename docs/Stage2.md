...
- Для каждого reference block решить: зарегистрирован сейчас, отсутствует из-за dependency, или нужен compatibility alias.
- Изменить registry names на legacy-compatible names, если нет зафиксированного 1.12-only compatibility решения.
...
**Риски / зависимости:**
Часть отсутствующих block classes является dependency на Stage 4/7 behavior. Stage 2 закрывает registry identity для реализованных blocks и документирует отсутствующие entries как later-stage content/behavior deferrals, не как обязательную задачу загрузки внешних сохранений.
...
- Проверить fresh-world placement/load для реализованных tile ids.
...
**Риски / зависимости:**
Некоторые tile classes относятся к Stage 4 behavior, но их ids являются Stage 2 registry compatibility. Если class отсутствует, Stage 2 нельзя закрывать как complete без documented deferral/compatibility decision.
...
- Rename 1.12 `ResourceLocation` paths only with an explicit compatibility decision; otherwise use names equivalent to reference lowercased only if required by Forge and documented.
...
- Confirm 1.12 handling of biome ids: if numeric ids cannot be hard-bound safely, document the no-op policy and still preserve keys/defaults.
...
**Риски / зависимости:**
Forge 1.12 biome numeric ids differ from 1.7.10; this requires explicit compatibility policy, not silent ignoring.

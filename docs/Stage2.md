...
### 9.2 WIP registry migration mappings

These mappings document the silent WIP ids replaced by canonical Stage 2 ids. Existing WIP worlds using the old snake_case ids require registry remapping or save migration; Stage 2 does not silently preserve both ids.
...
#### Representative item mappings and exceptions
...
Explicit Stage 2 item migration exceptions:

| Old WIP id | Canonical id | Migration reason |
|---|---|---|
...
Raw 1.7.10 NBT ids such as `TileJar` are not automatically equivalent to namespaced 1.12 ids such as `thaumcraft:tilejar`; offline old-save NBT remapping remains a documented migration task outside Stage 2 runtime registration.

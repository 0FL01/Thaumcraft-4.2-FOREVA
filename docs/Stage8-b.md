...
### Q1: Thaumometer scan feedback: Stage 8-b GUI или render/overlay?

**Resolution:** не считать это обычным `GuiScreen` scope. Reference указывает на item renderer/model assets (`ItemThaumometerRenderer`, `scanner.obj`, `scanscreen.png`), а не на обычный GUI screen. Видимый scan feedback относится к render/overlay/held-item visual scope, а не к Stage 8-b GUI routing.

Формулировка: "Thaumometer scan feedback should not be treated as a normal GUI screen requirement. Stage 8-b should ensure no-crash routing and any required client hooks/assets, while full visible feedback belongs to the renderer/overlay scope and must be tracked as a Stage 8 render dependency."

### Q2: Magic Box и Spa входят в Stage 8-b?

**Resolution:** да, если Stage 8-b заявляет полное GUI routing coverage. Текущий `ClientProxy.getClientGuiElement()` уже маршрутизирует Magic Box и Spa. Они являются частью текущей client GUI surface, даже если не были названы в "core user list". Оставшиеся gameplay-различия (linked storage для Magic Box, fluid placement для Spa) — это задачи Phase 4/server parity, а не GUI gap.

Формулировка: "Magic Box and Spa must be included in Stage 8-b GUI routing coverage because they are active current/common GUI IDs with client screens. If the checkpoint is narrowed, they must be explicitly deferred; otherwise Stage 8-b cannot claim full GUI routing completion."

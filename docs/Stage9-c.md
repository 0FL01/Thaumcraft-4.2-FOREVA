...
Reference `ConfigRecipes.initializeArcaneRecipes()` регистрирует 63 `ThaumcraftApi.addInfusionCraftingRecipe(...)` записи и кладет их в `ConfigResearch.recipes.put(...)`. Port now includes baseline helper `registerInfusionRecipe(...)` plus Stage 9-c subset registration in `ConfigRecipes.init()`:

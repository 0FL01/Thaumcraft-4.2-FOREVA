...
Reference `ConfigRecipes.initializeArcaneRecipes()` регистрирует 63 `ThaumcraftApi.addInfusionCraftingRecipe(...)` записи и кладет их в `ConfigResearch.recipes.put(...)`. Port now includes a first baseline helper `registerInfusionRecipe(...)` plus Stage 9-c wand-component subset registration in `ConfigRecipes.init()`:
...
These entries preserve reference keys/research/instability/aspect formulas and `ConfigResearch.recipes` handle writes, but full infusion coverage is still open.

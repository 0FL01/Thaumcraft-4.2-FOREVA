...
- Текущая registration (hub + infusion slice): `src/main/java/thaumcraft/common/config/ConfigRecipes.java` hub delegates to `ConfigRecipesInfusionSlice.java`, `ConfigRecipesInfusionDeviceSlice.java`, `ConfigRecipesInfusionEquipmentSlice.java`.
...
Главный блокер был в отсутствии infusion content data. Сейчас `ConfigRecipes.init()` (hub) вызывает slice-методы в `ConfigRecipesInfusionSlice`, `ConfigRecipesInfusionDeviceSlice` и `ConfigRecipesInfusionEquipmentSlice`, которые регистрируют reference infusion crafting и enchantment рецепты. Current scan подтверждает наличие всех 63 `addInfusionCraftingRecipe` и 24 `addInfusionEnchantmentRecipe` registrations, а также `InfusionRunicAugmentRecipe`.

Из-за отсутствующих data registrations Stage 9-c нельзя считать завершенной: Infusion Matrix может искать рецепты, но список пуст для всех Thaumcraft infusion outputs. Дополнительно отсутствует dynamic class `src/main/java/thaumcraft/common/lib/crafting/InfusionRunicAugmentRecipe.java`, `ConfigResearch.recipes` map is populated by `ConfigRecipes.init()` (see Stage9-d.md Q2 resolution), отсутствуют research entries/pages для прямого unlock/display flow, и не проведена ручная runtime проверка altar scenarios.

## 5. Gap list

### GAP-1: Регистрация infusion crafting recipes неполная

**Статус:** частично закрыт (wand caps/rods baseline added)  
**Критичность:** blocker

**Текущая реализация:**
- `src/main/java/thaumcraft/common/config/ConfigRecipes.java` hub delegates to `ConfigRecipesInfusionSlice.java`
- `src/main/java/thaumcraft/common/config/recipes/ConfigRecipesInfusionSlice.java`
- `src/main/java/thaumcraft/common/config/recipes/ConfigRecipesInfusionDeviceSlice.java`
...
Reference `ConfigRecipes.initializeArcaneRecipes()` регистрирует 63 `ThaumcraftApi.addInfusionCraftingRecipe(...)` записи и кладет их в `ConfigResearch.recipes.put(...)`. Port now includes baseline helper `registerInfusionRecipe(...)` plus Stage 9-c subset registration in the infusion slice files, called from hub `ConfigRecipes.init()`:
...
Port the reference infusion crafting registration block into the 1.12.2 infusion slice flow (called from hub `ConfigRecipes.init()`), preserving research keys, `ConfigResearch.recipes` map keys, output stacks/NBT object outputs, instability values, AspectList costs, central input, component order and conditional config gates.

**Как доделать:**
- Add 1.12.2 imports and registration code in `src/main/java/thaumcraft/common/config/recipes/ConfigRecipesInfusionSlice.java` (and companion slices for device/equipment).
...
**Критерии приемки:**
- [x] Hub `ConfigRecipes.init()` now calls infusion slices which register all 63 reference infusion crafting recipes.
...
Current API can construct `InfusionEnchantmentRecipe`, and the matrix can look for them. Port now includes helper-based registration in `ConfigRecipesInfusionSlice` (called from hub `ConfigRecipes.init()`) for the full reference key set:
...
**Как доделать:**
- Add all `ThaumcraftApi.addInfusionEnchantmentRecipe` calls to `src/main/java/thaumcraft/common/config/recipes/ConfigRecipesInfusionSlice.java`.
...
Hub `ConfigRecipes.init()` (via `ConfigRecipesInfusionSlice`) now adds `new InfusionRunicAugmentRecipe()` into `ThaumcraftApi.getCraftingRecipes()` with an idempotence guard, and `TileInfusionMatrix.craftingStart` now special-cases `InfusionRunicAugmentRecipe` to use `getComponents(recipeInput)` for dynamic pedestal ingredient capture as in reference.
...
Reference recipe registration stores every infusion recipe handle in `ConfigResearch.recipes` under stable IDs, then research pages can reference those handles and unlock/display recipes. Current `ConfigResearch.init()` is the current research registration baseline (see Stage9-e.md) and `ConfigResearch.recipes` map is populated by `ConfigRecipes.init()` (see Stage9-d.md Q2 resolution). Current `ResearchPage` has constructors for infusion recipes and infusion enchantment recipes, but nothing creates pages that reference actual infusion recipe objects (`src/main/java/thaumcraft/api/research/ResearchPage.java:84`, `src/main/java/thaumcraft/api/research/ResearchPage.java:90`).
...
Ensure the `ConfigResearch.recipes` registry contains all infusion recipe handles and create infusion research pages so registered infusion recipes have stable handles, appear in Thaumonomicon pages and are gated by matching research keys.

**Как доделать:**
- Ensure `ConfigResearch.recipes` contains all Stage 9-c infusion recipe handles with reference semantics.
- Confirm that `ConfigRecipes.init()` stores every Stage 9-c infusion recipe in `ConfigResearch.recipes` using reference keys.
...
**Критерии приемки:**
- [ ] `ConfigResearch.recipes` contains all Stage 9-c infusion recipe IDs from reference.
...
- [x] Port all 63 reference infusion crafting recipe registrations into infusion slices (called from hub `ConfigRecipes.init()`).
...
### Q1: Repair/Haste enchantments

**Resolution:** нужны реальные `Enchantment` instances текущего порта; не заменять ванильными аналогами. Reference использует `ThaumcraftApi.enchantRepair` и `ThaumcraftApi.enchantHaste`; текущий порт должен предоставить зарегистрированные enchantments с устойчивыми `ResourceLocation`.

Формулировка: "`Repair` and `Haste` infusion enchantment recipes must use real mod-registered `Enchantment` instances. If the current port does not yet register them, those recipes are blocked. Do not substitute vanilla enchantments unless the reference behavior explicitly changed."

### Q2: Mirror config field

**Resolution:** сохранить семантику `Config.allowMirrors`. Если поле в текущем порте отсутствует — добавить equivalent config с тем же default/meaning, а не размазывать condition по recipes.

Формулировка: "Mirror, MirrorHand, and MirrorEssentia recipes should be gated by the current-port equivalent of reference `Config.allowMirrors`. If no equivalent exists, add one and document it as the compatibility gate."

### Q3: Проверка outputs/components

**Resolution:** обязательный registry audit до закрытия Stage 9-c. Все infusion outputs, components, instability values, research keys, aspects, и enchantment references должны резолвиться против текущих 1.12.2 registries.

Формулировка: "All infusion outputs, components, instability values, research keys, aspects, and enchantment references must resolve against current 1.12.2 registries before Stage 9-c is accepted. Missing outputs are dependency blockers, not optional omissions."

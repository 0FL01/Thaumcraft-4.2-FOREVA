...
### Checkpoint 2026-05-21 — first-person thaumometer regression rollback
...
- `./scripts/dev.sh compileJava` — passed.
- `./scripts/dev.sh gradle test --rerun-tasks --tests thaumcraft.client.ThaumometerItemRendererContractTest --tests thaumcraft.common.lib.network.playerdata.PlayerDataPacketClientBoundaryStaticGuardTest --tests thaumcraft.common.lib.research.ScanManagerThaumometerNotificationStaticGuardTest --tests thaumcraft.common.lib.research.ScanManagerValidScanRuntimeTest --tests thaumcraft.common.lib.network.playerdata.PacketScannedAuthorityRuntimeTest` — passed.
- `./scripts/dev.sh validate --smoke` — passed.
...
**Checkpoint 2026-05-21 — thaumometer TC6 donor positioning (replaces custom first-person arm render):**
The earlier transform-aware route still relied on a custom first-person arm-rendered scanner posture with manual reflection-based equipped-progress lookups. Live visual feedback showed the arm posture was impractical to calibrate — the TC4.2 `scanner.obj` front face is `-Z` while the TC6 `scanner.obj` front face is `+X`, so any hand-rendered positioning required unbounded coordinate tweaking. The current port cuts the custom first-person path entirely: `ThaumometerPerspectiveModel.handlePerspective()` no longer returns a `null` matrix for first-person contexts, and `ItemThaumometerRenderer` no longer has `renderFirstPersonSetup()`, `renderFirstPersonHands()`, `applyContextTransform()`, or reflection-based equipped-progress fields. Instead, the renderer applies a universal TC4.2→TC6 basis adapter (`TC4_TO_TC6_VERTICAL_CENTER = -0.1F`, `TC4_TO_TC6_Y_ROTATION = -90.0F`) that rotates the port's scanner mesh into the TC6 orientation, then delegates all perspective transforms — including first-person — to Forge display transforms sourced from Thaumcraft 6 donor JSON (`itemthaumometer_tesr.json`). The donor first-person Z offset was then user-calibrated to `-14.352` for comfortable handheld distance. This gives us true TC6-match positioning without unbounded manual tweaking. Manual client validation is still required before claiming the whole thaumometer surface closed, but the architectural approach is now donor-aligned and no longer requires ad-hoc per-context calibration.
...
**Checkpoint 2026-05-21 — thaumometer scan targeting/client feedback parity pass:**
The scan gameplay route had drifted independently from the render route: `ItemThaumometer` and `ItemThaumometerRenderer` each carried their own simplified target-selection copy, and both had lost the older permissive entity-target path that allowed non-collidable dropped items to be scanned. The port now restores the 1.7.10-style pointed-entity route (`getPointedEntity(..., 0.5, 10.0, 0.0f, true)`) inside the thaumometer, splits raw target acquisition from validated scan completion so renderer/HUD and packet authority share one source of truth, and restores `Thaumcraft.proxy.blockRunes(...)` feedback for valid entity/block/node scan candidates while the player is actively scanning. Runtime validation remains non-GUI only, but the dropped-item scan route is no longer intentionally missing from the port.

**Checkpoint 2026-05-21 — thaumometer HUD/aspect notifications restored:**
Follow-up live testing showed the scan route now completed correctly but the client still looked dead: the first-person scanner overlay only rendered during active use, the readout had been reduced to a single `vis` line instead of the reference aspect grid, and `PacketAspectDiscovery` / `PacketAspectPool` silently mutated capability state without the original corner notifications. The current port restores a minimal reference-compatible client presentation layer: `ItemThaumometerRenderer` now keeps the first-person readout live while the scanner is held, renders scanned aspect tags through a restored `UtilsFX.drawTag(...)` helper, and resolves dropped-item names correctly. A compact `PlayerNotifications` + `REHNotifyHandler` pair is back on the overlay event path, and the aspect discovery/pool packets once again emit localized scan gain notifications plus orb feedback on the client. This is still non-GUI validated only until a real client rerun confirms the exact look-and-feel, but the previously missing aspect/message surface is now wired again.

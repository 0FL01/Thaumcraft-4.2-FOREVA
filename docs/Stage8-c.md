...
- No open question blocks implementation planning. The main uncertainty is not scope but porting approach per renderer: some reference 1.7.10 renderers use removed APIs (`IIcon`, `ISimpleBlockRenderingHandler`, `IModelCustom`, old Tessellator calls), so each implementation checkpoint must inspect/decompile the specific reference class before porting.
- Dependency: if Stage 8-d/e owns shader/particle helpers, Stage 8-c TESRs that reference node halos, crucible bubbles or infusion beams must either degrade gracefully or document the exact visual part deferred to Stage 8-d/e; this does not make the core TESR absence acceptable.
- Dependency: if Stage 9 recipes/research are still missing, manual scenarios should use creative placement, test commands or direct tile NBT/state to validate renderers rather than waiting for progression.

...
Remaining mismatch: the original `spawn*BossRoom(...)` methods also mutate the boss room with large decorative/gameplay block templates, urn/crate scatter, obelisks, traps, taint/biome patches, and sparkle packets. The current checkpoint restores the server trigger, world-data update, and boss entity spawn anchors only; full boss-room decoration and runtime traversal/save-reload evidence remain unverified.

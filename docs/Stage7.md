...
- Решение по legacy numeric biome IDs: для Forge 1.12.2 идентичность биомов фиксируется registry names, а не numeric IDs. Старые config keys из `src/main/java/thaumcraft/common/config/Config.java:41-45` нужно читать как legacy/no-op для миграционной прозрачности, но не пытаться принудительно назначать numeric IDs. В closure notes Stage 7 явно указать эту политику и стабильные registry names.
- Решение по portal trigger: collision-driven teleport в `src/main/java/thaumcraft/common/blocks/BlockEldritchPortal.java:48-78` считать временной реализацией, а не parity-решением. Закрывать GAP-3 через reference-like trigger в `TileEldritchPortal` и safe `TeleporterThaumcraft`: поиск/caching порталов, проверка null target world, сброс velocity, cooldown против loops, прибытие рядом с существующим portal block из `src/main/java/thaumcraft/common/lib/world/dim/GenPortal.java:135-138`.
- Решение по недостающим room blocks: `blockLootUrn`, `blockLootCrate`, `blockSlabStone` закрывать в Stage 7 как часть server-side traversal/room parity, а не silently defer. Допустимы только явно документированные cosmetic deferrals после runtime-проверки, что progression/traversal/loot не ломаются. Assets брать из `thaumcraft_src/assets/`.
- Дополнительный blocker к плану закрытия: перед признанием GAP-4/GAP-10 закрытыми нужно привести `BlockEldritch` и связанные tile entities к meta/TE контракту, который реально используют Outer Lands rooms. Сейчас генераторы ставят metas `7/8/9/10` для door/lock/crab spawner/trap через `src/main/java/thaumcraft/common/lib/world/dim/GenCommon.java:166-202`, но `src/main/java/thaumcraft/common/blocks/BlockEldritch.java:20-22` ограничен metas `0..4`, а часть eldritch tile entities остается stub.
...
1. Config/checkpoint:
   - читать legacy biome ID config keys как documented no-op;
...
5. Worldgen/maze/checkpoint:
   - восстановить normal/retrogen split в `ThaumcraftWorldGenerator`;
   - подключить `ServerTickEventsFML` retrogen queue к реальному retrogen path;

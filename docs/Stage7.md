...
- Current surface generator has server-side baselines for Outer Lands, Nether, wild aura nodes, vegetation, ores, flowers, totems, structure-nearby nodes, and surface structures in `ThaumcraftWorldGenerator`, but runtime evidence and exact distribution parity remain open.
- Current Overworld structure/tree generators have server-side baselines for hilltop altars, mound/barrows, Eldritch rings, and spider Greatwoods after the 2026-05-14 checkpoints, but they still need fresh-world runtime evidence, exact-template review where documented, and broader surface worldgen parity validation.
...
### GAP-1: Surface worldgen pipeline needs runtime and edge-case parity evidence

**Статус:** частично реализовано
**Критичность:** blocker

**Текущая реализация:**
- `src/main/java/thaumcraft/common/lib/world/ThaumcraftWorldGenerator.java:258-620`
...
**Что не совпадает:**
Reference `ThaumcraftWorldGenerator` has a `worldGeneration(random, chunkX, chunkZ, world, newGen)` path that handles Outer Lands, Nether, End skip, surface generation, and generation markers. The current port now has server-side baselines for that control flow, including wild aura nodes, structure-nearby nodes, totems, Nether generation, vegetation, ores, flowers, surface structures, retrogen marker execution, and Outer Lands room ownership. Remaining mismatch is runtime and edge-case evidence: fresh-world and retrogen lifecycles have not reached a ready server, biome/dimension blacklist behavior has not been observed in-world, and exact distribution parity for ore/structure placement still needs scenario validation.

**Что нужно доделать:**
Validate and refine the restored reference-like worldgen control flow, preserving config toggles, dimension/biome blacklist semantics, flat-world skip behavior, Nether node/totem behavior, surface aura generation, structure-node generation, vegetation, ores, structures, and chunk dirty marking where appropriate.

**Как доделать:**
- Keep `worldGeneration(Random, int, int, World, boolean)` as the shared fresh-generation and retrogen entry point.
- Verify `generateWildNodes`, `generateTotem`, `generateSurface`, `generateVegetation`, `generateOres`, and `generateNether` against runtime scenarios and reference edge cases.
- Verify that normal `blockAiry`/`TileNode` aura nodes can be created independently of Silverwood trees.
...
Remaining GAP-1 limits after this checkpoint: `newGen`/regen chunk dirty-marker parity, broader biome blacklist/runtime edge cases, and runtime evidence are still open.

**Checkpoint 2026-05-14 — Retrogen newGen marker execution:**
`ThaumcraftWorldGenerator` now exposes a reference-like `worldGeneration(Random, int, int, World, boolean newGen)` entry point. Fresh Forge worldgen calls it with `newGen=true`; queued chunk regeneration calls it with `newGen=false`. The generator now gates aura, structures, trees, ores, and Nether generation as `Config.genX && (newGen || Config.regenX)` instead of letting regen toggles create content in fresh chunks when the matching generation toggle is disabled. Non-fresh generation marks the chunk dirty after the generation pass, matching the reference marker intent.

`ServerTickEventsFML` now actually drains queued retrogen chunks by re-seeding the chunk RNG and calling `Thaumcraft.instance.worldGen.worldGeneration(..., false)`. `EventHandlerWorld` now avoids adding duplicate `ChunkLoc` entries when the same missing-marker chunk is loaded more than once before the queue drains.

Remaining GAP-1 limits after this checkpoint: runtime proof is still blocked by the smoke environment; broader biome blacklist/runtime edge cases remain open until fresh-world and retrogen scenarios can be observed.
...
Remaining GAP-2 limits after this checkpoint: runtime generation has not been observed in a fresh world, async maze race/saving behavior is still unproven, and altar activation/opening remains dependent on tile behavior checkpoints and runtime validation.
...
Remaining altar limits after this checkpoint: eye insertion and portal-opening behavior are handled by the next altar checkpoint, but cultist/guardian spawn behavior has not been observed in a runtime world.

**Checkpoint 2026-05-14 — Eldritch altar eye and portal activation:**
`BlockEldritch` meta `0` now accepts `ItemEldritchObject` meta `0` while the player is not sneaking and the altar has fewer than four eyes. It increments `TileEldritchAltar.eyes`, starts the guardian spawner phase once at least two eyes were already present, calls `checkForMaze()`, consumes the eye item, syncs the tile, and plays the original crystal sound.

`TileEldritchAltar` now implements the wand activation path for the portal-opening step. A wand right-click opens the altar only when `OCULUS` research is complete, the altar has exactly four eyes, it is not already open, a dark `TileNode` is above it, `checkForMaze()` succeeds, and the wand can pay the six-primal 100-vis crafting cost. Success sets `open`, removes the dark node, places `blockEldritchPortal` above the altar, syncs the tile, and plays the wand sound.

Remaining altar limits after this checkpoint: generated ring activation has not been observed in a fresh runtime world, and the async case where `checkForMaze()` has just started a `MazeThread` still needs runtime/save validation before GAP-2/GAP-3 can close.
...
**Checkpoint 2026-05-14 — Boss room mutation parity:**
`TileEldritchLock` now restores the reference server-side boss-room mutation routines before spawning each selected boss. Warden rooms place the offset obelisks/caps, urn scatter, trap/cosmetic center supports, and offset stair/crystal pedestal. Golem rooms place three obelisks/caps, the central pedestal, and corner-biased urn/crate scatter. Cultist rooms place the randomized eldritch floor and perimeter pillar/slab layout. Taint rooms patch the 25x25 biome area, grow taint fibres on exposed air, scatter taint blocks, and place the central taint crust pattern.

Remaining GAP-4 limits after this checkpoint: key-room generation has not been observed in a runtime Outer Lands maze, full portal/passage/nest/library/boss room traversal remains unvalidated, and boss-room mutations still need runtime/save evidence.
...
**Что не совпадает:**
Config keeps original numeric biome ID fields (`biomeTaintID`, `biomeMagicalForestID`, `biomeEerieID`, `biomeEldritchID`) at `src/main/java/thaumcraft/common/config/Config.java:41-45` and now reads the old keys in `syncConfigurable()` at `src/main/java/thaumcraft/common/config/Config.java:258-262`. Current Forge 1.12.2 biome registration still uses registry names only, so these numeric values are preserved as legacy config identity/documented no-ops and are not used to force biome ids. That is the active Forge 1.12-compatible policy for this port, but runtime validation of biome registration/decorators is still required. PRD also calls biome color/debug overlay cosmetics a known risk: `docs/PRD.md:352-358`. Current biome color overrides exist, but no runtime/client verification is recorded.

**Что нужно доделать:**
Keep documenting the 1.12.2-compatible policy for numeric biome IDs, and verify biome colors/spawns/decorators under runtime. Do not silently break config identity.

**Как доделать:**
- Numeric biome ID config keys are read and retained as legacy documented no-ops; do not attempt to force numeric ids in Forge 1.12.2 biome registration.
- Keep registry-name/no-op policy in Stage 7 closure notes and final status reports.
...
**Критерии приемки:**
- [x] Biome registry names are stable and documented as the Forge 1.12 identity policy.
- [x] Numeric biome ID config keys are read and explicitly documented as a Forge 1.12 registry-name/no-op policy.
...
**Что не совпадает:**
`MapBossData` itself matches the simple reference field/key shape (`bossCount`). The 2026-05-14 lock checkpoint wires the reference server-side lifecycle baseline: `BlockEldritch` meta `8` accepts `ItemEldritchObject` meta `2`, starts the lock countdown, `TileEldritchLock` persists/syncs `facing` and `count`, plays the countdown sounds, loads/creates `BossMapData`, advances `bossCount` with the original 25% extra increment, chooses the boss type by `bossCount % 4`, clears nearby `blockAiry`, and removes the lock. Current block placement cancellation now checks `Config.dimensionOuterId`, so the previously documented hard-coded `-42` safety bypass is resolved in current source.

The 2026-05-14 boss-room mutation checkpoint restores the server-side decorative/gameplay block mutation paths from the original `spawn*BossRoom(...)` methods: Warden/Golem obelisks, caps, pedestals, urn/crate scatter, traps/supports, Cultist floor/pillars, and Taint biome/fibre/block patches. Remaining mismatch: the reference also sends sparkle packets while clearing nearby airy door blocks, which is Stage 8/client FX work. Runtime traversal/save-reload evidence remains unverified.
...
**Checkpoint 2026-05-14 — Eldritch lock boss trigger baseline:**
`BlockEldritch` now handles the original lock activation path for meta `8` blocks with `ItemEldritchObject` meta `2`. `TileEldritchLock` now extends `TileThaumcraft` for custom NBT/update sync, keeps the original `count = -1` inactive state, ticks the 100-tick pump countdown, plays pump/ice sounds, updates `BossMapData`, chooses the boss by the reference modulo cycle, spawns Warden/Golem/Cultist Portal/Taint bosses at reference-like room anchors, clears nearby airy door blocks, and removes the lock. Warden/Golem initial spawn hooks are called so Warden title/warding setup is not skipped.

Remaining GAP-10 limits after this checkpoint: the large reference boss-room block decoration routines are not fully ported, boss fights have not been observed in a fresh runtime Outer Lands maze because server smoke remains environment-blocked, and save/reload boss progression has not been manually validated.

**Checkpoint 2026-05-14 — Boss room mutation parity:**
`TileEldritchLock` now mutates the boss room before entity spawn using the reference server-side block placement logic. The checkpoint covers Warden/Golem obelisks/caps/pedestals/loot scatter, Warden center traps/supports, Cultist floor and perimeter pillar/slab layout, and Taint biome/fibre/block patching. This closes the static block-template mismatch called out above, but not runtime room traversal, boss fight behavior, save/reload evidence, or client sparkle packets for the cleared airy door blocks.

### GAP-11: Overworld hilltop altar and mound/barrow structures need runtime/exact-template evidence

**Статус:** частично реализовано
**Критичность:** high

**Текущая реализация:**
- `src/main/java/thaumcraft/common/lib/world/ThaumcraftWorldGenerator.java:500-536`
- `src/main/java/thaumcraft/common/lib/world/WorldGenHilltopStones.java`
- `src/main/java/thaumcraft/common/lib/world/WorldGenMound.java`
...
**Что не совпадает:**
The hilltop altar and mound/barrow are no longer the small placeholder structures described by the original GAP-11 audit. `WorldGenHilltopStones` now has the reference-like validation, altar/stones layout, dungeon-loot chest, and `thaumcraft:wisp` spawner baseline. `WorldGenMound` now has the 19x19 validation points, buried chamber, loot urn/crate placements, dungeon-loot chest, trapped-chest/TNT chance, and skeleton/zombie spawners. Remaining mismatch is that the mound shell is a compact functional equivalent rather than the exact large fixed reference block dump, and neither structure has been observed in a fresh runtime world.

**Что нужно доделать:**
Validate the current templates in fresh-world generation and decide whether the compact mound shell needs to be replaced with the exact fixed reference block dump. These are Stage 7 gameplay structures because they affect world exploration, loot, mob spawning, and aura/node placement.

**Как доделать:**
- Keep `WorldGenHilltopStones` and `WorldGenMound` in the reference-like shared `randPosY = getHeight(...) - 9` structure branch.
- Runtime-check that the hilltop altar still refuses invalid surfaces and generates its loot chest and `thaumcraft:wisp` spawner.
- Runtime-check that the mound/barrow generates its chamber, loot urn/crate placements, populated chest, trapped-chest/TNT chance, and skeleton/zombie spawners.
- Compare the compact mound shell against the exact reference block dump only after runtime proves the functional layout is stable.
...
Remaining GAP-11 limits after this checkpoint were the mound/barrow generator, broader `generateSurface(...)` chance semantics, and runtime evidence. The mound/barrow and shared chance-ordering baselines were advanced by the follow-up checkpoint below; runtime evidence remains blocked.
...
- Решение по numeric biome IDs: для Forge 1.12.2 идентичность биомов фиксируется registry names, а не numeric IDs. Старые config keys из `src/main/java/thaumcraft/common/config/Config.java:41-45` уже читаются в `syncConfigurable()` как documented no-op; не пытаться принудительно назначать numeric IDs. В closure notes Stage 7 явно сохранять эту политику и стабильные registry names.
...
- Дополнительный blocker к плану закрытия: direct `BlockEldritch` meta/TE contract закрыт отдельным checkpoint: блок больше не clamp'ит generated room metas `7/8/9/10` в `0..4` и создает нужные tile entities. `TileEldritchAltar` теперь имеет server-side spawner/checkForMaze/eye/portal-opening baseline, но runtime traversal/save evidence остаются открытыми. `TileEldritchCrabSpawner` и `TileEldritchTrap` имеют server-side tick baseline; client FX/runtime evidence остаются открытыми. `TileEldritchLock` теперь имеет server-side activation/countdown/boss-spawn and boss-room block mutation baseline; оставшиеся lock/boss-room риски — runtime traversal, save/reload evidence, and Stage 8 client sparkle/render parity.
...
1. Config/checkpoint:
   - numeric biome ID config keys уже читаются как documented no-op;
...
5. Worldgen/maze/checkpoint:
   - validate restored normal generation control flow in `ThaumcraftWorldGenerator`;
   - validate Eldritch ring bootstrap: reference-like width/height/seed, sea-level/surface validation, full altar layout и dark node/altar behavior;
   - validate `WorldGenHilltopStones`/`WorldGenMound` templates: validation, filled loot, `Thaumcraft.Wisp`/skeleton/zombie spawners, urn/crate placements;
   - validate normal/spider Greatwood generation path: non-Magical-Forest vegetation chance, `CaveSpider` spawner, webs, and loot chest;

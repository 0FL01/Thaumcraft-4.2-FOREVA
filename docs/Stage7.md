...
- Current portal collision still actively teleports players in `BlockEldritchPortal`, while the safer `TeleporterThaumcraft` search/cache placement is only a partial GAP-3 closure until the reference ticked `TileEldritchPortal` flow and manual traversal evidence are restored.
...
Remaining GAP-3 limits after this checkpoint: portal placement/search is safer, but the main trigger still lives in `BlockEldritchPortal.onEntityCollision(...)` instead of the reference ticked `TileEldritchPortal` flow, and no manual portal traversal/save-reload validation has been run.
...
Remaining GAP-4 limits after this checkpoint: other room templates still need reference audit, full room traversal has not been runtime/manual validated, and loot-table distribution remains dependent on Stage 9/content registration.
...
- Решение по portal trigger: safe `TeleporterThaumcraft` placement/search/caching частично закрыт отдельным checkpoint, но collision-driven teleport в `src/main/java/thaumcraft/common/blocks/BlockEldritchPortal.java:48-78` считать временной реализацией, а не parity-решением. Полное закрытие GAP-3 требует reference-like trigger в `TileEldritchPortal`, проверки null target world, cooldown против loops, и runtime evidence прибытия рядом с существующим portal block из `src/main/java/thaumcraft/common/lib/world/dim/GenPortal.java:135-138`.
...
- Дополнительный blocker к плану закрытия: direct `BlockEldritch` meta/TE contract закрыт отдельным checkpoint: блок больше не clamp'ит generated room metas `7/8/9/10` в `0..4` и создает нужные tile entities. Полное поведение этих tile entities (`TileEldritchAltar`, `TileEldritchLock`, `TileEldritchCrabSpawner`, `TileEldritchTrap`) остается открытым до отдельной parity-проверки.
...
1. Config/checkpoint:
   ...
   - заменить hard-coded `-42` checks на `Config.dimensionOuterId`;
   - зафиксировать registry-name policy для биомов в closure notes.
2. Portal/checkpoint:
   - перенести основной teleport trigger в tickable `TileEldritchPortal` по reference flow;
   - `BlockEldritchPortal.onEntityCollision` удалить или оставить только как строго guarded fallback;

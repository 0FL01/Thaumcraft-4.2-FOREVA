...
### Q1: JSON или Java `IRecipe`?

**Resolution:** оба, по правилу сложности. JSON использовать только для простых shaped/shapeless рецептов без NBT, dynamic output, ore dictionary quirks, subtype-sensitive behavior и custom matching. Java `IRecipe` нужен для рецептов с NBT, сохранением/изменением данных itemstack, ore dictionary, dynamic result, metadata-sensitive behavior или нестандартным matching. Arcane/crucible/infusion recipes не надо насильно превращать в vanilla JSON — они должны идти через Thaumcraft API / текущие recipe registries.

Формулировка: "Simple vanilla workbench recipes should be JSON. Any recipe with NBT, ore dictionary, dynamic output, metadata-sensitive custom matching, or Thaumcraft-specific mechanics should remain Java-backed and be recorded in the recipe audit."

### Q2: Как проверять meta/subtype definitions?

**Resolution:** только по текущей 1.12.2 реализации, не по reference metas вслепую. Надо сделать canonical mapping для `ItemResource`, `ItemNugget`, `ItemShard`, `BlockCustomOre`, `BlockCustomPlant`, `BlockCosmeticSolid/Opaque` и остальных subtype-heavy объектов.

Формулировка: "Metadata values must be verified against the current port's item/block subtype definitions before recipes are migrated. Reference metadata may guide the audit but must not be treated as authoritative for the 1.12.2 registry."

### Q3: Что делать с отсутствующими outputs?

**Resolution:** не пропускать молча и не подменять "похожими" items. Каждый рецепт должен иметь статус: implemented, blocked by missing output, blocked by missing ingredient, blocked by missing mechanic, или deferred with owner.

Формулировка: "Missing current-port outputs must be recorded per recipe with the exact missing registry object or metadata dependency. Do not silently omit recipes or substitute unrelated outputs."

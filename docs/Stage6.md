...
- Нужно подтвердить точную compatibility policy для 1.7.10 entity id/spawn egg migration на 1.12.2 registry names; это не заменяет GAP-12, а определяет приемлемую реализацию.
- Нужно решить, относится ли `BlockLoot` к Stage 6 implementation или к dependency from content/block phase; для Cultist Portal это прямой blocker независимо от владельца файла.
- Нужно определить минимальный формат ручной Stage 6 scenario matrix, чтобы runtime evidence был воспроизводимым и не сводился к общему smoke-тесту.

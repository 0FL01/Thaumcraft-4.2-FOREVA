...
**Resolution:** держать Thaumcraft API arcane recipe creation в `ConfigRecipes.init()` / recipe config layer; Forge `IRecipe` — через registry event. Это особенно важно, потому что текущий код уже имеет `ConfigRecipes.init()`, который чистит и заполняет `ConfigResearch.recipes`, а также вызывает baseline arcane/infusion/crucible init blocks.

...
Статус: **незакрыт**. Задокументировано **42 checkpoint'а** (2026-05-16 – 2026-05-17),
все — focused bursts по entity rendering, каждая запись содержит статус, что сделано,
проверки и ограничения. Большинство checkpoint'ов имеют статус «частично продвинут».

### Что было сделано (суммарно)

- **Bootstrap**: GAP-1 coverage, asset corpus из `thaumcraft_src/assets/thaumcraft/`
- **Fallback layers**: item-like, projectile, vanilla mob, taint animal, cultist biped, monster, special entities
- **Coverage guards**: `ClientProxyEntityRendererCoverageByConfigTest`
- **Dedicated renderers**: pech, firebat, trunk, golem (type-texture), wisp (aspect-tint),
  eldritch guardian/warden/golem/crab, cultist portal, thaumic slime
- **Taint entity renderers**: spore trio, taintacle trio, brainy/inhabited zombie, mind/taint spider,
  creeper (flash/armor), villager, chicken (wing), cow, pig, sheep (fur-layer)
- **Server-contract restoration**: sheep sheared/AI, creeper armor/underwater/NBT/attack/explosion/spread/FX,
  taint chicken/cow/pig/sheep/villager (AI/attributes/drops), swarm/spore lifecycle,
  taintacle small/giant/core, mind spider (viewer/sync/translucent/collision),
  inhabited zombie armor/spawn
- **Projectile replacements**: aspect orb, electric orb, explosive orb, ember, eldritch orb,
  primal orb, pech blast, alumentum, dart, primal arrow, golem bobber, falling taint, frost shard
...
### Ключевые незакрытые блоки

- GAP-1..GAP-7 всё ещё имеют открытые пункты
- Visual parity не подтверждена клиентским рантаймом
- Stage 6 entity-server contracts остаются зависимостью для некоторых entity renderers

Полный лог: `docs/GOAL_PROGRESS-archive/stage8-d-checkpoints.md`
...
### Q1: Port/register `RenderWatcher`?

**Resolution:** нет, не в Stage 8-d, пока common/content scope не зарегистрирует `EntityWatcher` как entity. Текущий `ConfigEntities` не содержит `EntityWatcher` в entity registration, только упоминание в champion whitelist. Это надо трактовать как content/common gap или dangling whitelist reference, а не как renderer gap.

Формулировка: "Do not port or register `RenderWatcher` as part of Stage 8-d unless `EntityWatcher` is first registered by the common/content entity scope. Current source only references `EntityWatcher` in the champion whitelist, which is not enough to make a renderer valid or spawnable."

### Q2: Можно ли подтвердить visual parity статикой?

**Resolution:** нет. Статический аудит может подтвердить registration coverage, missing classes, resource paths, side safety и obvious GL mistakes. Но parity надо проверять в клиенте: spawn matrix, screenshots, logs, no missing texture spam, no GL state bleed.

Формулировка: "Static analysis can close renderer registration coverage, but not visual parity. Stage 8-d acceptance requires runtime/manual client verification for representative entities."
